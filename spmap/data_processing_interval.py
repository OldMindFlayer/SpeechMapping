...
path_to_experiment_data = 'C:/Workspace/SpeechMapping/data/SeleznevaFull2/11_03_20/experiment_data/experiment_data.h5'
path_to_results = 'C:/Workspace/SpeechMapping/data/SeleznevaFull2/11_03_20/experiment_data/experiment_data.png'
...
for pair in PAIRS:
    ...
    for ch in range(ECOG[i].shape[1]):
        ...
        if( BAD_CH[i][ch] | BAD_CH[j][ch] ):
            ...
            stim_i = np.zeros((STIM[i].shape[0],1));
            stim_j = np.zeros((STIM[j].shape[0],1));
            stim_i[:,0]      = np.copy(STIM[i]); 
            stim_j[:,0]      = np.copy(STIM[j]); 
            ...
            if( (ind_stim_i.shape[0]==0) & (ind_stim_j.shape[0]==0)):
                ...
                indj = np.zeros(0);
                for k  in ind_stim_j:
                    indj = np.append(indj, np.array(range( k+int(INTERVAL_START*srate),k+int(INTERVAL_STOP*srate))));
                indi = np.array(range(0,min(ecog_i.shape[0],indj.shape[0])));
            elif((ind_stim_i.shape[0]!=0) & (ind_stim_j.shape[0]==0)):      
                indi = np.zeros(0);
                for k  in ind_stim_i:
                    indi = np.append(indi, np.array(range( k+int(INTERVAL_START*srate),k+int(INTERVAL_STOP*srate ))));
            ...
            for f in range(len(fbandmins)):
                ...
                xi = filterEMG(ecog_i,fbandmin,fbandmax,srate)[indi]
                xj = filterEMG(ecog_j,fbandmin,fbandmax,srate)[indj]
                x = np.vstack((xi,xj));
               
                y = np.vstack((np.ones((xi.shape[0],1))*VALS[i],
                   np.ones((xj.shape[0],1))*VALS[j]));
